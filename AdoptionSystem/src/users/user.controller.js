...
export const deleteU = async(req, res)=>{
    try {
        ...
        //Validar si esta logrado y es el mismo 

        //Eliminar (deleteOne/findOneAndDelete)
        let deleteUser = await User.findOneAndDelete({_id: id})

        //Verificar que se eliminó
        if(!deleteUser) return res.status({messaje: 'Account not found and not deleted'})
        //Responder
    return res.send({messaje: `account with username ${deleteUser.username} deleted success`})
    } catch (err) {
        console.error(err)
